...
class ChoreTracker(db.Model):
    ...
    due_by_date = db.Column(db.Date, nullable=True)  # optional; if null, due = date (no late penalty by default)
    ...
    def to_dict(self):
        return {
            ...
            'due_by_date': self.due_by_date.isoformat() if self.due_by_date else None,
            'status': self.status,
