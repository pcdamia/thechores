...
def create_tracker_entry():
    ...
    if data.get('due_by_datetime'):
        # Expect ISO format datetime string
        due_by_datetime = datetime.fromisoformat(data['due_by_datetime'].replace('Z', '+00:00'))
    elif data.get('due_by_date') and data.get('due_by_time'):
        # Backward compat: combine date and time
        date_part = datetime.strptime(data['due_by_date'], '%Y-%m-%d').date()
        time_part = datetime.strptime(data['due_by_time'], '%H:%M').time()
        due_by_datetime = datetime.combine(date_part, time_part)
    elif data.get('due_by_date'):
        # Old format: just date, set to end of day
        date_part = datetime.strptime(data['due_by_date'], '%Y-%m-%d').date()
        due_by_datetime = datetime.combine(date_part, datetime.max.time())
    
    tracker = ChoreTracker(
        chore_id=data.get('chore_id'),
        date=tracker_date,
        due_by_datetime=due_by_datetime,
...
def update_tracker_entry(tracker_id):
    ...
    data = request.json
    if 'due_by_datetime' in data:
        val = data.get('due_by_datetime')
        tracker.due_by_datetime = datetime.fromisoformat(val.replace('Z', '+00:00')) if val else None
    elif 'due_by_date' in data or 'due_by_time' in data:
        # Handle date/time separately
        date_val = data.get('due_by_date')
        time_val = data.get('due_by_time')
        if date_val and time_val:
            date_part = datetime.strptime(date_val, '%Y-%m-%d').date()
            time_part = datetime.strptime(time_val, '%H:%M').time()
            tracker.due_by_datetime = datetime.combine(date_part, time_part)
        elif date_val:
            date_part = datetime.strptime(date_val, '%Y-%m-%d').date()
            tracker.due_by_datetime = datetime.combine(date_part, datetime.max.time())
        else:
            tracker.due_by_datetime = None
    if 'frequency' in data:
        tracker.frequency = data.get('frequency')
    if 'assigned_user_id' in data:
        tracker.assigned_user_id = data.get('assigned_user_id')
    if 'room_id' in data:
        tracker.room_id = data.get('room_id')
    ...
    if 'status' in data:
        ...
        if chore:
            ...
            if new_status == 'completed' and tracker.approved_by_id:
                today = date.today()
                due_date = (tracker.due_by_datetime.date() if tracker.due_by_datetime else None) or tracker.date
